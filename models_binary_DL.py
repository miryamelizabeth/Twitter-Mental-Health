...
DISORDER_TAG = 'DIAGNOSED'
BINARY_DvsC = True # Flag for diagnosed vs control
# ---------------------------
...
def load_dataset(trainFile, testFile):
	...
	train = train[train['class'].isin(['CONTROL', DISORDER_TAG])]
	test = test[test['class'].isin(['CONTROL', DISORDER_TAG])]
...
def run_model(dataDirectory, embeddingsDirectory, saveDirectory, nameResults, language, method):
	...
	for i in range(1, FOLDS + 1):
		...
		classifiers_list = {'CNN': CNN_model(embedding_matrix),
							...
							'CNN-LSTM': CNN_LSTM_model(embedding_matrix)
						}
